...
public class PieChart extends View {
	...
	public ArrayList<Integer> getColor()
	{
		...
		String[] legend_colors = getResources().getStringArray(R.array.colors);
		int colorSize = legend_colors.length;
		
		for(int i = 0; i < iDataSize; i++) {
			if (i>=colorSize){
				colors.add(i, Color.parseColor(legend_colors[i%colorSize]));
			}else{
				colors.add(i, Color.parseColor(legend_colors[i]));
			}	
		}
